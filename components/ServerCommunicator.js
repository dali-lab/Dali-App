   ...
   loggedIn(user) {
     return new Promise(function (resolve, reject) {
       ...
       GoogleSignin.currentUserAsync((user) => {
         ...
         this.post(`${env.serverURL}/api/signin`, {
   ...
   getTonightsLabHours() {
     ...
     return new Promise((resolve, reject) => {
       this.getLabHours().then((hours) => {
         ...
         taHours.sort((hour1, hour2) => hour1.startDate > hour2.startDate);
     ...
     return new Promise((success, failure) => {
       GoogleSignin.currentUserAsync().then((user) => {
         if (user != null) {
           ...
           if (this.accessToken === undefined) {
             return GoogleSignin.hasPlayServices({ autoResolve: true }).then(() => GoogleSignin.currentUserAsync()).then(user => RNGoogleSignin.getAccessToken(user)).then((token) => {
               this.accessToken = token;
         ...
         fetch(`https://www.googleapis.com/calendar/v3/calendars/${user != null ? env.eventsCalendarId : env.publicEventsCalendarId}/events${user === null ? `?key=${env.googleConfig.publicKey}` : ''}`, {
           ...
           .then((responseJson) => {
             ...
             events.sort((event1, event2) => (event1.startDate > event2.startDate ? 1 : -1));
