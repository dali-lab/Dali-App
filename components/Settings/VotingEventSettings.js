...
class VotingEventSettings extends Component {
   ...
   editOptionDescription(option) {
     if (option.awards === null || option.awards.length === 0) {
       ...
       Alert.alert('Add or Remove?', 'Add or remove an award?', [
         ...
         { text: 'Remove',
           onPress: () => {
             const buttons = option.awards.map((award, index) => ({ text: award,
               onPress: () => {
         ...
         if (event === null) {
           ...
           newEvent.options.forEach((option) => {
             ...
             if (corespondingIndex === -1) {
               ...
               const localOption = event.options[corespondingIndex];
   ...
   releasePressed() {
     ...
     ServerCommunicator.current.releaseAwards(this.state.event).then(() => {
       ...
       Alert.alert('Encountered an error!', error.message, { text: 'Ok',
         onPress: () => {
           this.setState({
             showCoverModal: false
           });
     ...
     return (
       ...
       >
         <View>
           ...
           {option.awards != null ?
             option.awards.map(award => <Text key={award} style={styles.rowAwardText}>Award: {award}</Text>)
             :
             null
           }
